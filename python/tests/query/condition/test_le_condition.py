...
class TestLeCondition:
    ...
    def test_le_condition_type_miss_match(self):
        assert_type(c(User).id == "1", TypeMissMatch[int, str])

    @pytest.mark.parametrize("value", [True, False, None])
    def test_le_condition_use_is(self, value: bool | None):
        assert_type(c(User).id == value, UseIsInsteadOfEq)

    @pytest.mark.parametrize("value", [1, None])
    def test_le_condition_use_nullable(self, value: int | None):
        # NOTE: Consider whether null safety can be supported
        assert_type(c(User).age <= value, LeCondition)
